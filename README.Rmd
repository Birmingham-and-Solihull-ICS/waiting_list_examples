...
+ `example_of_sim.R` - example sim showing my simulation route to estimate them simulate a waiting list.
+ `waiting_times_distribution.R` - plot showing the proportion of compliant patients at 18weeks, given different average waits.
+ `WL_factor_vs_probability_compliance` - Shiny app, showing how `factor` argument interacts with % compliance, and calculates a target queue size.
...
This repository contains an interactive Shiny application that visualises the
relationship between compliance (expressed as a percentage) and the exponential
distribution factor required to meet a waiting list target.
...
or clone within RStudio, Positron or similar and run interactively.
You can view the deployed app at: https://chrismainey.shinyapps.io/wl_factor_vs_probability_compliance/
